...
class anelasticity_model(object):
   ...
   def vs_2_thermo(self, depths, seis_model):
      """
      ...
      #dens_fld = [None]*len(seis_model);
      #visc_fld = [None]*len(seis_model);
      dens_fld_fromT = [None]*len(seis_model);
      visc_fld_fromT = [None]*len(seis_model);
      
      # compute seismic velocities
      self.vs, self.logvisc, self.rho, _, _, _ = self.compute_vs_vsu_anel_qual\
      ...
      for i in range(len(seis_model)):
         ...
         #dens_fld[i] = np.sum((1./seis_tree_res[0])*self.rho[i][seis_tree_res[1]],axis=1)/\
         #         np.sum(1./seis_tree_res[0],axis=1);
         #visc_fld[i] = np.sum((1./seis_tree_res[0])*self.logvisc[i][seis_tree_res[1]],axis=1)/\
         #         np.sum(1./seis_tree_res[0],axis=1);
         z = self.dpths[i]/1.e3 # km
         T_solidus = load_solidus(dpths=np.array([z]), mode='const_grad', solgrad=self.solgrad, T50=self.sol50)
         pres = np.array([z*(1./30.)*1e9])
         dens_fld_fromT[i] = density_fld_ga(temp_fld[i], pres, grun=grun, a0=a0, a1=a1,\
                 dens_ref=rhom, K0=K0, KT=KT, A=AY, C=CY);
         visc_fld_fromT[i] = self.function_viscosity(temp_fld[i], T_solidus, pres,\
         ...
         #dens_fld[i] = dens_fld[i].reshape(orig_shape);
         #visc_fld[i] = visc_fld[i].reshape(orig_shape);
         dens_fld_fromT[i] = dens_fld_fromT[i].reshape(orig_shape);
         visc_fld_fromT[i] = visc_fld_fromT[i].reshape(orig_shape);

      return temp_fld, dens_fld_fromT, visc_fld_fromT
   ...
   def compute_vs_vsu_anel_qual(self, depths, temps):
      ...
      return Vs, np.log10(viscosity), self.rho, Vsu, anel_corr, qual_fac
   ...
   def function_viscosity(self, temps, T_solid, pres, T_eta, back_visc, act_eng, act_vol):
      ...
      inputs:
         temps:      an array of the possible tempereature
         T_solid:    solidus temperature at each depth 
         pres:       pressure array, which in this case can be translated into depth
...
def density_fld_ga(temps, pres, grun, a0, a1, dens_ref, K0, KT, A, C):
   ...
   """
   temps_x, pres_x  = np.meshgrid(temps, pres);
   # using quadratic parameterisation of pressure-dependent expansivity (otherwise, use slower brent optimization)
   ...
   #        dV_x[i,j]=res.x;
   const_a = -7.334963115431564676e-23;
   const_b = 7.510867653681621105e-12;
   const_c = 1.000184023114681908e+00;
   dV_x = (const_a*pres_x**2) + (const_b*pres_x) + const_c;
