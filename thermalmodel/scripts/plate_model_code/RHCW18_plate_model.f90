...
!   real(wp), parameter ::  e_crust =226.
  real(wp), parameter ::  a_crust =0.432
  real(wp), parameter ::  b_crust =0.44
  real(wp), parameter ::  c_crust =380.
  real(wp), parameter ::  d_crust =0.305
  real(wp), parameter ::  e_crust =145.
...
! ------------------------------------------------------------------------  
  ...
  subroutine plate_cooling(Tpot,nz)
    ...
    character(50)  ::  hffile
    character(50)  ::  Tinfile
    character(50)  ::  Tgridfile
    character(50)  ::  depthfile
    
    ! set file names
    if (nz<100) then
      write (hffile, "('../../data_output/hf-',I4,'-',I2,'-',I4,'.dat')")Tpot,zp,rd ! output file for heat flow
      write (depthfile, "('../../data_output/depth-',I4,'-',I2,'-',I4,'.dat')")Tpot,zp,rd ! output file for subsidence
      write (Tinfile, "('initial_geotherms/mac',I4,'.dat')")Tpot ! initial temperature profile
      write (Tgridfile, "('../../data_output/Tgrid-',I4,'-',I2,'-',I4,'.dat')")Tpot,zp,rd ! output file for thermal structure
    else 
      write (hffile, "('../../data_output/hf-',I4,'-',I3,'-',I4,'.dat')")Tpot,zp,rd ! output file for heat flow
      write (depthfile, "('../../data_output/depth-',I4,'-',I3,'-',I4,'.dat')")Tpot,zp,rd ! output file for subsidence
      write (Tinfile, "('initial_geotherms/mac',I4,'.dat')")Tpot ! initial temperature profile
      write (Tgridfile, "('../../data_output/Tgrid-',I4,'-',I3,'-',I4,'.dat')")Tpot,zp,rd ! output file for thermal structure
    end if
...
subroutine correction(T1,T2,T3,cora,alphaP0,rhoP0)
    ...
    ! finds optional dpCp/dt correction term used in McKenzie et al. 2005 eqn. 3
