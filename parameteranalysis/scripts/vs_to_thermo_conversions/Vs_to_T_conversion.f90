...
777	  continue
	...
	fail=0
	open(1,file='input.zVs')
	j=1
	do while(j.le.readmax)
	   read (1,*,end=888) z(j), Vs(j)
	   call Vs_calc(z(j),Vs0,T0,Qout,etaout,rhoout)
       if(Vs(j).gt.Vs0) then
         T(j)=0.
	     etao(j)=1.e40
	     Qo(j)=1.e-9
	     fail=fail+1
	     print *, "number of fails (too fast for sensible T) is", fail
	   else
	     call T_calc(z(j),Vs(j),T(j),Qo(j),etao(j),rhoo(j))
	   end if
	   j=j+1
	end do
888	continue
	...
	open(2,file='output_Vs_to_T.zTVsQvd')	
	i=1
	do while (i.le.k)
...
!  Difference between observed and calculated temperature
    call Vs_calc(zT,Vsout,T,Q,eta,rho)
    fvs=abs(VsT-Vsout)
    return
  end function fvs
!--------------------------------------------------
! Brent to iteratively minimise misfit between calculated and predicted temperature
!--------------------------------------------------
  function brent_TfromVs(bax,bbx,bcx,f,btol,bxmin)   
  	use commondat
  	use extradat
  	use refdat

    integer, parameter :: bitmax=500
    real, parameter :: bCGOLD=.3819660
    real, parameter :: bZEPS=1.0e-10
    integer :: biter
    real :: ba,bb,bdd,be,betemp,bfu,bfv,bfw,bfx,bp,bq,br,btol1,btol2,bu,bv,bw,bx,bxm
	external f
...
!     exactly zero.
!     write(*,*) 'calling brent for Vs to T conversion'

    ba=min(bax,bcx)        !a and b must be in ascending order, though the input
    bb=max(bax,bcx)    !abscissas need not be.
    bv=bbx          !Initializations...
    bw=bv
    bx=bv
    be=0.    !This will be the distance moved on the step before last.
    bfx=f(bx)
    bfv=bfx
    bfw=bfx
    do biter=1,bitmax   !Main program loop.
      bxm=0.5*(ba+bb)
      btol1=btol*abs(bx)+bZEPS
      btol2=2.*btol1
      if(abs(bx-bxm).le.(btol2-.5*(bb-ba))) goto 3    !Test for done here.
      if(abs(be).gt.btol1) then        !Construct a trial parabolic fit.
        br=(bx-bw)*(bfx-bfv)
        bq=(bx-bv)*(bfx-bfw)
        bp=(bx-bv)*bq-(bx-bw)*br
        bq=2.*(bq-br)
        if(bq.gt.0.) bp=-bp
        bq=abs(bq)
        betemp=be
        be=bdd
	if(abs(bp).ge.abs(.5*bq*betemp).or.bp.le.bq*(ba-bx).or. bp.ge.bq*(bb-bx)) goto 1
	!The above conditions determine the acceptability of the parabolic fit. Here it is o.k.:
	bdd=bp/bq !Take the parabolic step.
	bu=bx+bdd
	if(bu-ba.lt.btol2 .or. bb-bu.lt.btol2) bdd=sign(btol1,bxm-bx)
	goto 2     !Skip over the golden section step.
      endif
1     if(bx.ge.bxm) then    !We arrive here for a golden section step, which we take
        be=ba-bx     !into the larger of the two segments.
      else
        be=bb-bx
      endif
      bdd=bCGOLD*be           !Take the golden section step.
2     if(abs(bdd).ge.btol1) then     !Arrive here with dd computed either from parabolic fit, or
        bu=bx+bdd              !else from golden section.
      else
        bu=bx+sign(btol1,bdd)
      endif
      bfu=f(bu) !This is the one function evaluation per iteration,
      if(bfu.le.bfx) then    !and now we have to decide what to do with our function
        if(bu.ge.bx) then     !evaluation. Housekeeping follows:
          ba=bx
        else
          bb=bx
        endif
        ...
        if(bfu.le.bfw .or. bw.eq.bx) then
	  bv=bw
	  bfv=bfw
	  bw=bu
	  bfw=bfu
	else if(bfu.le.bfv .or. bv.eq.bx .or. bv.eq.bw) then
	  bv=bu
	  bfv=bfu
	endif
...
3   bxmin=bx    !Arrive here ready to exit with best values.
    brent_TfromVs=bfx
    return
    end function brent_TfromVs
!--------------------------------------------------
...
!--------------------------------------------------
! Calculate temperature from Vs
!--------------------------------------------------
subroutine T_calc(z,Vs,T,Q,eta,rho)
 	use transdat
	use meltdat
	use commondat
	use extradat
...
! 	external fvs
	real :: T0, Vsl, Vsu, Q, Ql, Qu, eta, etal, etau, fitmin, xmin, tau
    real :: rho, rhou, rhol
	zT=z
	VsT=Vs
	A=0.
	...
	BX=1000.
	tol=1.e-4
	fitmin=brent_TfromVs(AX,BX,CX,fvs,tol,xmin)
	T=xmin
	call Vs_calc(z,Vsl,(T-5.),Ql,etal,rhol)
	call Vs_calc(z,Vsu,(T+5.),Qu,etau,rhou)
    dVsdT=(Vsl-Vsu)/10.
    call Vs_calc(z,Vsu,(T),Q,eta,rho)
	return
