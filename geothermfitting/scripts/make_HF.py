...
fold_HF = os.path.join(fold_data_output, folds["output_HF"])
fold_HF_latest = os.path.join(folds["data_output"], f'Tp_{potential_temperature}_sol50_{solidus_50km}', 'latest', folds["output_HF"])

os.makedirs(fold_HF, exist_ok=True)
os.makedirs(fold_HF_latest, exist_ok=True)
...
if fitfile_type.strip().lower() in ['map', 'maximum a posteriori', 'most probable']:
    ...
    outfold = os.path.join(fold_HF, 'MAP')
    outfold_latest = os.path.join(fold_HF_latest, 'MAP')
    os.makedirs(outfold, exist_ok=True)
    os.makedirs(outfold_latest, exist_ok=True)
    outfile = os.path.join(outfold, f'{model}_HF_MAP.txt')
    outfile_latest = os.path.join(outfold_latest, f'{model}_HF_MAP.txt')
    ...
    outfold = os.path.join(fold_HF, 'distribution')
    outfold_latest = os.path.join(fold_HF_latest, 'distribution')
    os.makedirs(outfold, exist_ok=True)
    os.makedirs(outfold_latest, exist_ok=True)
    ...
    os.makedirs(outfold, exist_ok=True)
    os.makedirs(outfold_latest, exist_ok=True)
    outfile = os.path.join(outfold, f'{model}_HF_{fitfile_index}.txt')
    outfile_latest = os.path.join(outfold_latest, f'{model}_HF_{fitfile_index}.txt')
