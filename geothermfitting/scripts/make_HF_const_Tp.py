...
fold_HF_const_Tp = os.path.join(fold_data_output, folds["output_HF_const_Tp"])
fold_HF_const_Tp_latest = os.path.join(folds["data_output"], f'Tp_{potential_temperature}_sol50_{solidus_50km}', 'latest', folds["output_HF_const_Tp"])

os.makedirs(fold_HF_const_Tp, exist_ok=True)
os.makedirs(fold_HF_const_Tp_latest, exist_ok=True)
...
if fitfile_type.strip().lower() in ['map', 'maximum a posteriori', 'most probable']:
    ...
    outfold = os.path.join(fold_HF_const_Tp, 'MAP')
    outfold_latest = os.path.join(fold_HF_const_Tp_latest, 'MAP')
    os.makedirs(outfold, exist_ok=True)
    os.makedirs(outfold_latest, exist_ok=True)
    outfile = os.path.join(outfold, f'{model}_HF_const_Tp_MAP.txt')
    outfile_latest = os.path.join(outfold_latest, f'{model}_HF_const_Tp_MAP.txt')
    ...
    outfold = os.path.join(fold_HF_const_Tp, 'distribution')
    outfold_latest = os.path.join(fold_HF_const_Tp_latest, 'distribution')
    os.makedirs(outfold, exist_ok=True)
    os.makedirs(outfold_latest, exist_ok=True)
    ...
    os.makedirs(outfold, exist_ok=True)
    os.makedirs(outfold_latest, exist_ok=True)
    outfile = os.path.join(outfold, f'{model}_HF_const_Tp_{fitfile_index}.txt')
    outfile_latest = os.path.join(outfold_latest, f'{model}_HF_const_Tp_{fitfile_index}.txt')
...
tp=np.full(np.size(cnt_map_data),1419.) # in const Tp case should use same Tp as where params came from
...
###############################################################################################
# 7. LOOP OVER LOCATIONS IN GRID AND CALCULATE LAB1200 AT EACH POINT
###############################################################################################
...
def main():
    ...
    for zl in range(np.shape(temp_arr)[2]):
        ...
        t_end=time.time()
        print("location:", zl, locs[zl,:], "time:", t_end-t_start, "completion:", np.round(100*(zl/np.shape(temp_arr)[2]),3), "%")
...
HFout=np.zeros((np.shape(temp_arr)[2], 13))
...
for fmt_idx in range(0,4):
    format_string += '%3.1f '
for fmt_idx in range(4,12):
    format_string += '%12.5f'
    format_string += ' '
format_string += '%12.5f'


np.savetxt(outfile, HFout, fmt=format_string)
np.savetxt(outfile_latest, HFout, fmt=format_string)
