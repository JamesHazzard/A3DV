...
cp $HOMEDIR/scripts/make_HF.py batch_start_foo_end_bar
...
for i in $(seq foo 1 bar); do
  python3 make_HF.py -v distribution -i ${i}
done
...
for startrun in $(seq $minnumber $chunksize $maxnumber); do
    ...
    cp concept junk
	sed "s/foo/$startrun/g" junk | sed "s/bar/$endrun/g" | sed "s/zoo/$model/g" > send_HF_${startrun}_${endrun}_${model}.sh

    # Make script executable
	chmod +x send_HF_${startrun}_${endrun}_${model}.sh

    # Submit chunk of LAB runs
	qsub ./send_HF_${startrun}_${endrun}_${model}.sh
