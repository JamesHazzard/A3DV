...
fold_LAB1200_latest = os.path.join(folds["data_output"], f'Tp_{potential_temperature}_sol50_{solidus_50km}', 'latest', folds["output_LAB1200"])

os.makedirs(fold_LAB1200, exist_ok=True)
os.makedirs(fold_LAB1200_latest, exist_ok=True)
...
if fitfile_type.strip().lower() in ['map', 'maximum a posteriori', 'most probable']:
    ...
    outfold_latest = os.path.join(fold_LAB1200_latest, 'MAP')
    os.makedirs(outfold, exist_ok=True)
    os.makedirs(outfold_latest, exist_ok=True)
    outfile = os.path.join(outfold, f'{model}_LAB1200_MAP.txt')
    outfile_latest = os.path.join(outfold_latest, f'{model}_LAB1200_MAP.txt')
    ...
    outfold_latest = os.path.join(fold_LAB1200_latest, 'distribution')
    os.makedirs(outfold, exist_ok=True)
    os.makedirs(outfold_latest, exist_ok=True)
    ...
    outfile_latest = os.path.join(outfold_latest, f'{model}_LAB1200_{fitfile_index}.txt')
...
kmod=np.full(np.shape(cnt_map_data),7)
kmod[cnt_map_data==cnt_map_data]=7
...
h=np.full(np.shape(cnt_map_data),1.0e-6)
h[cnt_map_data==cnt_map_data]=0.
h=np.reshape(h,np.size(cnt_map_data))
...
tp=np.full(np.size(cnt_map_data),1333.)
...
np.savetxt(outfile_latest, LABout, fmt='%3.1f %3.1f %3.1f')